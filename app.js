...
function checkResetDia(){
  // IMPORTANTE: Aguatero NO reinicia el reparto por cambio de fecha.
  // El reparto solo se reinicia cuando el usuario confirma “Cerrar reparto”.
  // Esta función queda como compatibilidad con versiones anteriores.
...
function verificarResetDiario(){
  ...
  // El estado del reparto permanece intacto hasta “Cerrar reparto”.
  // La fecha solo se actualiza al cerrar explícitamente el reparto.
  return false;
}
...
function construirEstadoActual(){
  return {
    ...
    deudaGeneradaHoy, envasesEntregadosHoy, envasesRecibidosHoy, envasesRecibidosB20Hoy, envasesRecibidosB10Hoy, envasesRecibidosSodaHoy,
...
function aplicarEstadoDesdeObjeto(estado){
  ...
  envasesRecibidosSodaHoy = estado.envasesRecibidosSodaHoy || 0;
...
function construirMensajeCierreDia(){
  ...
  const vaciosSoda = n(stockCamion && stockCamion.vaciosSoda);
  const totalVacios = vaciosB20 + vaciosB10 + vaciosSoda;
  ...
  return [
    ...
    'Sodas: ' + n(stockCamion && stockCamion.soda)
  ].join('\n');
}

function abrirWhatsAppConTexto(texto){
  const url = 'https://wa.me/?text=' + encodeURIComponent(texto);
  // Usamos un enlace real, igual que los botones de WhatsApp que ya funcionan
  // en la PWA. El click conserva el gesto del usuario en Android.
...
// Botón específico dentro del modal de "Cerrar reparto".
...
if (btnAgregar) btnAgregar.addEventListener('click', abrirModalNuevoCliente);
const btnAgregarTop = document.getElementById('btnAgregarTop');
if (btnAgregarTop) btnAgregarTop.addEventListener('click', abrirModalNuevoCliente);
...
function confirmarStock(tipo){
  ...
  const envSoda =
    parseInt(document.getElementById('stkEnvSoda').value) || 0;

  // Envases de bidones: afectan los envases pendientes del cliente.
  const envases = envB20 + envB10;
  // Total físico de envases vacíos recuperados (incluye sodas).
  const envasesRecibidosTotal = envB20 + envB10 + envSoda;
  ...
  const entry = {
    ...
    // Envases de bidones para calcular lo que el cliente debe
    envases: envases,
    // Total de envases vacíos recibidos, incluyendo sodas
    envasesRecibidosTotal: envasesRecibidosTotal,
  ...
  // Los envases que el cliente devuelve vuelven físicamente al camión.
  // No se limitan por el stock que la app tenga registrado del cliente.
...
function aplicarEfectoMovimiento(c, entry){
  if(entry.tipo === 'compra'){
    ...
    if(entry.fechaISO === todayISO()){
      ...
      envasesRecibidosSodaHoy += (entry.envSoda || 0);
      envasesRecibidosHoy += (entry.envasesRecibidosTotal != null ? entry.envasesRecibidosTotal : entry.envases);
...
function formatearMovimiento(entry){
  ...
  if(entry.soda) items.push(`${entry.soda} soda${entry.soda === 1 ? '' : 's'}`);
  ...
  if(entry.envSoda) vacios.push(`${entry.envSoda} envase de soda`);
  const detalleVacios = vacios.length ? ' · recibió ' + vacios.join(' + ') : '';
  return `Compró ${detalleItems} ($${entry.costo}) · ${etiquetaPago}${detalleVacios}`;
}
...
function abrirEditarMovimiento(clienteId, entryId){
  ...
  if(entry.tipo === 'compra'){
    ...
    document.getElementById('editEnvases').value = entry.envases || 0;
...
function guardarEdicionMovimiento(){
  ...
  const viejo = entry.tipo === 'compra' ? {
    b20: entry.b20||0, b10: entry.b10||0, disp: entry.disp||0, soda: entry.soda||0,
    envB20: entry.envB20||0, envB10: entry.envB10||0, envSoda: entry.envSoda||0
  ...
  if(nuevoTipo === 'compra'){
    ...
    const costo = b20*c.precio + b10*(c.precio10||0) + disp*(c.precioDisp||0) + soda*(c.precioSoda||0);
    let montoPagado = 0;
    ...
    entry.b20 = b20; entry.b10 = b10; entry.disp = disp; entry.soda = soda;
    entry.bidones = b20 + b10 + disp;
    entry.envases = envases;
    entry.envB20 = entry.envB20 || 0; entry.envB10 = entry.envB10 || 0; entry.envSoda = entry.envSoda || 0;
    entry.envasesRecibidosTotal = entry.envB20 + entry.envB10 + entry.envSoda;
    entry.costo = costo;
  ...
  if(entry.fechaISO === todayISO()){
    ...
    stockCamion.vaciosB20 = Math.max(0, (stockCamion.vaciosB20||0) + (entry.envB20||0) - (viejo.envB20||0));
    stockCamion.vaciosB10 = Math.max(0, (stockCamion.vaciosB10||0) + (entry.envB10||0) - (viejo.envB10||0));
    stockCamion.vaciosSoda = Math.max(0, (stockCamion.vaciosSoda||0) + (entry.envSoda||0) - (viejo.envSoda||0));
  }
...
function anularMovimiento(){
  ...
  if(entry.tipo === 'compra' && entry.fechaISO === todayISO()){
    ...
    // Al anular, también salen del stock del camión los envases que
    // habían sido recibidos en esa venta.
...
function renderStockCamion(){
  ...
  var elVaciosB20 = document.getElementById('stockVaciosB20');
  var elVaciosB10 = document.getElementById('stockVaciosB10');
  if(elVaciosB20) elVaciosB20.textContent = stockCamion.vaciosB20 || 0;
  if(elVaciosB10) elVaciosB10.textContent = stockCamion.vaciosB10 || 0;
  var elVaciosSoda = document.getElementById('stockVaciosSoda');
  if(elVaciosSoda) elVaciosSoda.textContent = stockCamion.vaciosSoda || 0;
  // Total de vacíos recuperados: incluye 20 L + 10–12 L + sodas.
  var totalVacios = (stockCamion.vaciosB20 || 0) + (stockCamion.vaciosB10 || 0) + (stockCamion.vaciosSoda || 0);
  var elVaciosTotal = document.getElementById('stockEnvasesVaciosHoy');
  if(elVaciosTotal) elVaciosTotal.textContent = totalVacios;
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Resumen">
  ...
  const nombreArchivo = `aguatero_movimientos_${nombreDia}_${fecha}.xls`;

  // Android/PWA: primero intentamos el selector nativo de guardado cuando
  // el navegador lo soporta. No usamos setTimeout: debe conservarse el
  // gesto original del usuario sobre el botón.
  if('showSaveFilePicker' in window){
    window.showSaveFilePicker({
      ...
      mostrarToast('✅ Excel guardado correctamente.', 'success');
    }).catch(err=>{
      if(err && err.name !== 'AbortError') descargarExcelFallback(blob, nombreArchivo);
    });
    return;
  }

  // Para exportar Excel priorizamos la descarga. Compartir por Web Share
  // puede abrir WhatsApp/Drive en lugar de guardar el archivo, por eso no lo
  // usamos como camino principal.
  descargarExcelFallback(blob, nombreArchivo);
}

function descargarExcelFallback(blob, nombreArchivo){
  const url = URL.createObjectURL(blob);

  let panel = document.getElementById('excelDownloadFallback');
  if(panel) panel.remove();
  panel = document.createElement('div');
  panel.id = 'excelDownloadFallback';
  ...
  panel.innerHTML = '<div style="font-weight:700;margin-bottom:8px;">📊 Excel listo</div><div style="font-size:.85em;margin-bottom:12px;">Si la descarga automática no comenzó, podés descargarlo o abrirlo desde el botón de abajo.</div>';

  const link = document.createElement('a');
  link.href = url;
  link.download = nombreArchivo;
  link.rel = 'noopener';
  link.textContent = '⬇️ Descargar Excel';
  link.className = 'btn verde';
  link.style.cssText = 'display:inline-block;text-decoration:none;padding:10px 18px;';
  panel.appendChild(link);

  const openFile = document.createElement('button');
  openFile.textContent = '📄 Abrir archivo Excel';
  openFile.className = 'btn outline';
  openFile.style.cssText = 'display:block;margin:10px auto 0;padding:10px 18px;';
  openFile.onclick = ()=>window.open(url, '_blank');
  panel.appendChild(openFile);

  const close = document.createElement('button');
  close.textContent = 'Cerrar';
  close.style.cssText = 'display:block;margin:10px auto 0;border:0;background:none;padding:6px;color:#666;';
  close.onclick = ()=>panel.remove();
  panel.appendChild(close);
  document.body.appendChild(panel);

  // Intentamos la descarga automática manteniendo el gesto del usuario.
  const auto = document.createElement('a');
  auto.href = url;
  auto.download = nombreArchivo;
  auto.rel = 'noopener';
  ...
  // Dejamos el Blob vivo el tiempo suficiente para Android/PWA.
...
function renderAtendidos(){
  ...
  // Los atendidos se ordenan por la última atención del día:
  // el cliente atendido más recientemente aparece primero.
  // No usamos el orden de la ruta porque ese orden solo corresponde a
  // cómo estaba planificado el reparto, no al orden real de atención.
  function indiceUltimaAtencion(c){
    if(!Array.isArray(c.historial)) return -1;
    let indice = -1;
    for(let i=0; i<c.historial.length; i++){
      const h = c.historial[i];
      if(h && h.fechaISO === todayISO() && (h.tipo === 'compra' || h.tipo === 'no_compra')){
        indice = i;
      }
    }
    return indice;
  ...
  const filtrados = clientes
    ...
    .sort((a,b)=> indiceUltimaAtencion(b) - indiceUltimaAtencion(a));
...
function renderEstadisticas(){
  ...
  const estEnvB20 = document.getElementById('estEnvB20'); if(estEnvB20) estEnvB20.textContent = envasesRecibidosB20Hoy;
  const estEnvB10 = document.getElementById('estEnvB10'); if(estEnvB10) estEnvB10.textContent = envasesRecibidosB10Hoy;
  const estEnvSoda = document.getElementById('estEnvSoda'); if(estEnvSoda) estEnvSoda.textContent = envasesRecibidosSodaHoy;
...
function confirmarCerrarReparto(){
  ...
  // El cierre debe mostrar exactamente los mismos vacíos recuperados que
  // aparecen en Stock del camión. No usamos contadores históricos separados,
  // porque pueden quedar desfasados después de corregir un movimiento.
  const cierreEnvB20 = document.getElementById('cierreEnvB20');
  const cierreEnvB10 = document.getElementById('cierreEnvB10');
  const cierreEnvSoda = document.getElementById('cierreEnvSoda');
  const cierreEnvTotal = document.getElementById('cierreEnvTotal');
  const vaciosB20 = stockCamion.vaciosB20 || 0;
  const vaciosB10 = stockCamion.vaciosB10 || 0;
  const vaciosSoda = stockCamion.vaciosSoda || 0;
  const totalVacios = vaciosB20 + vaciosB10 + vaciosSoda;
  if(cierreEnvB20) cierreEnvB20.textContent = vaciosB20;
  if(cierreEnvB10) cierreEnvB10.textContent = vaciosB10;
  if(cierreEnvSoda) cierreEnvSoda.textContent = vaciosSoda;
  if(cierreEnvTotal) cierreEnvTotal.textContent = totalVacios;
...
function cerrarRepartoDelDia(){
  ...
  resumenesDiarios[fecha] = {
    ...
    envasesRecibidosB20: envasesRecibidosB20Hoy,
    envasesRecibidosB10: envasesRecibidosB10Hoy,
    envasesRecibidosSoda: envasesRecibidosSodaHoy,
  ...
  entregadoHoy = 0; deudaGeneradaHoy = 0; envasesEntregadosHoy = 0; envasesRecibidosHoy = 0; envasesRecibidosB20Hoy = 0; envasesRecibidosB10Hoy = 0; envasesRecibidosSodaHoy = 0;
  ...
  // El nuevo reparto queda marcado desde el momento del cierre explícito.
  fechaContadores = fecha;
  fechaInicioTrabajo = fecha;
