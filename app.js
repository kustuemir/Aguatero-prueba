...
async function descargarDatosSupabase(){
  ...
  try{
    ...
    if(stockData && (!stockCamion.b20 && !stockCamion.b10 && !stockCamion.disp)){
      ...
      stockCamion = { b20: stockData.b20 || 0, b10: stockData.b10 || 0, disp: stockData.disp || 0, vaciosB20: stockCamion.vaciosB20 || 0, vaciosB10: stockCamion.vaciosB10 || 0 };
    }
...
let stockCamion = { b20: 0, b10: 0, disp: 0, vaciosB20: 0, vaciosB10: 0 };
...
function aplicarEstadoDesdeObjeto(estado){
  ...
  b20VendidosHoy=estado.b20VendidosHoy || 0; b10VendidosHoy=estado.b10VendidosHoy || 0; dispVendidosHoy=estado.dispVendidosHoy || 0;
  visitasHoy=new Set(estado.visitasHoy || []);
  fechaContadores=estado.fechaContadores || todayISO();
  stockCamion=Object.assign({b20:0,b10:0,disp:0,vaciosB20:0,vaciosB10:0},estado.stockCamion||{});
...
function abrirModalMensajeTransferencia(){
  ...
  if(input) input.value=localStorage.getItem('mensajeTransferenciaPendiente') ||
...
function confirmarStock(tipo){
  ...
  const entry={id:generarId(),tipo:'compra',fechaISO:todayISO(),hora:ahora.toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'}),
    b20,b10,disp,envB20,envB10,envases,bidones,costo,formaPago:tipo,montoPagado,
...
function aplicarEfectoMovimiento(c,entry){
  if(entry.tipo==='compra'){
    ...
    c.envasesPendientes += (entry.bidones||0) - (entry.envases||0);
    ...
    if(entry.fechaISO===todayISO()){
      ...
      envasesEntregadosHoy += entry.bidones || 0;
      ...
      b20VendidosHoy += entry.b20||0; b10VendidosHoy += entry.b10||0; dispVendidosHoy += entry.disp||0;
    }
  }else if(entry.tipo==='pago'){
    if(entry.fechaISO===todayISO()) cobradoHoy += entry.montoPagado || 0;
  }
}
...
function abrirEditarMovimiento(clienteId,entryId){
  ...
  const e20=document.getElementById('editEnvB20'), e10=document.getElementById('editEnvB10');
  if(e20) e20.value=entry.envB20||0;
  if(e10) e10.value=entry.envB10||0;
  document.getElementById('editEnvases').value=entry.envases||((entry.envB20||0)+(entry.envB10||0));
...
function guardarEdicionMovimiento(){
  ...
  const viejo=entry.tipo==='compra'?{b20:entry.b20||0,b10:entry.b10||0,disp:entry.disp||0,envB20:entry.envB20||0,envB10:entry.envB10||0}:{b20:0,b10:0,disp:0,envB20:0,envB10:0};
  ...
  let nuevo={b20:0,b10:0,disp:0,envB20:0,envB10:0};
  if(nuevoTipo==='compra'){
    ...
    const e20El=document.getElementById('editEnvB20'), e10El=document.getElementById('editEnvB10');
    const envB20=e20El?parseInt(e20El.value)||0:parseInt(document.getElementById('editEnvases').value)||0;
    ...
    entry.b20=b20;entry.b10=b10;entry.disp=disp;entry.bidones=b20+b10;entry.envB20=envB20;entry.envB10=envB10;entry.envases=envases;
    entry.costo=costo;entry.formaPago=formaPago;entry.montoPagado=montoPagado;entry.transferenciaConfirmada=formaPago==='transferencia_pendiente'?false:undefined;
    nuevo={b20,b10,disp,envB20,envB10};
  }
  if(entry.fechaISO===todayISO()){
...
function renderStockCamion(){
  const elB20=document.getElementById('stockB20'), elB10=document.getElementById('stockB10'), elDisp=document.getElementById('stockDisp');
  ...
  const e20=document.getElementById('stockVaciosB20'), e10=document.getElementById('stockVaciosB10'), total=document.getElementById('stockEnvasesVaciosHoy');
  if(e20) e20.textContent=stockCamion.vaciosB20||0;
  if(e10) e10.textContent=stockCamion.vaciosB10||0;
  if(total) total.textContent=(stockCamion.vaciosB20||0)+(stockCamion.vaciosB10||0);
  const umbral=5;
  ...
  if(banner){
    ...
    if(bajos.length>0&&(stockCamion.b20>0||stockCamion.b10>0||stockCamion.disp>0)){banner.style.display='block';banner.textContent='⚠️ Stock bajo: '+bajos.join(', ');}
...
${rows.map(r=>row(r)).join('')}
  ...
  const nombre=`aguatero_movimientos_${diaDeHoy().toLowerCase()}_${todayISO()}.xls`;
...
function confirmarCerrarReparto(){
  ...
  set('cierreB20',b20VendidosHoy); set('cierreB10',b10VendidosHoy); set('cierreDisp',dispVendidosHoy);
  set('cierreEnvB20',stockCamion.vaciosB20||0); set('cierreEnvB10',stockCamion.vaciosB10||0);
  set('cierreEnvTotal',(stockCamion.vaciosB20||0)+(stockCamion.vaciosB10||0));
...
function cerrarRepartoDelDia(){
  ...
  resumenesDiarios[fecha]={fecha,venta:ventaHoy,efectivo:efectivoHoy,transferencia:transferenciaHoy,deuda:deudaGeneradaHoy,
    ...
    b20Vendidos:b20VendidosHoy,b10Vendidos:b10VendidosHoy,dispVendidos:dispVendidosHoy,
  ...
  b20VendidosHoy=0;b10VendidosHoy=0;dispVendidosHoy=0;visitasHoy=new Set();clientesFueraRutaHoy=new Set();
  stockCamion={b20:0,b10:0,disp:0,vaciosB20:0,vaciosB10:0};
  fechaContadores=fecha;fechaInicioTrabajo=fecha;modoTodosClientes=true;searchTerm='';
  document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));
